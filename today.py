...
def top_languages(top_n=3):
    """
    Uses GitHub's GraphQL v4 API to aggregate language byte-sizes across all of my owned
    repositories, then returns a short 'Python 52% · TS 24% · Java 12%' summary of the top N.
    """
    abbrev = {'TypeScript': 'TS', 'JavaScript': 'JS', 'Jupyter Notebook': 'Jupyter'}
    totals, cursor = {}, None
    while True:
        ...
        request = simple_request(top_languages.__name__, query, {'login': USER_NAME, 'cursor': cursor})
        repos = request.json()['data']['user']['repositories']
        for repo in repos['edges']:
            for lang in repo['node']['languages']['edges']:
                totals[lang['node']['name']] = totals.get(lang['node']['name'], 0) + lang['size']
    ...
    return ' · '.join(f"{abbrev.get(name, name)} {round(size / grand_total * 100)}%" for name, size in top)


def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data, lang_data):
    """
    ...
    justify_format(root, 'lang_data', lang_data)
...
if __name__ == '__main__':
    """
    Adapted for Vortrix5 (Mohamed Amine Zouaoui), 2026
    """
    ...
    # "Uptime" is measured from your GitHub account creation date so no manual config is needed.
    # To show your real age instead, replace the next line with:
    #     age_data, age_time = perf_counter(daily_readme, datetime.datetime(YYYY, M, D))
    acc_birth = datetime.datetime.strptime(acc_date, '%Y-%m-%dT%H:%M:%SZ')
    age_data, age_time = perf_counter(daily_readme, acc_birth)
    ...
    lang_data, lang_time = perf_counter(top_languages, 3)
    formatter('top languages', lang_time)

    for index in range(len(total_loc) - 1):
        total_loc[index] = '{:,}'.format(total_loc[index])  # format added, deleted, and total LOC

    svg_overwrite('dark_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1], lang_data)
    svg_overwrite('light_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1], lang_data)
